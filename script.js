...
let lastWordCreation = Date.now();
const MAX_WORDS = 4;
const WORD_CREATION_INTERVAL = 2000; // 2 seconds
...
function gameLoop() {
    if (!gameOver) {
        ...
        const now = Date.now();
        if (now - lastWordCreation > WORD_CREATION_INTERVAL && currentWords.length < MAX_WORDS) {
            if (createWord()) {
